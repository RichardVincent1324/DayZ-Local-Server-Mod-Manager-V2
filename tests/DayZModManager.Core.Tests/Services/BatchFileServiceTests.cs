...
public class BatchFileServiceTests
{
    ...
    [Fact]
    public void ReadModList_ParsesSingleLineWithPathsAndTrailingSemicolon()
    {
        var fs = new FakeFileSystem();
        fs.AddFile(BatPath, "set \"modList=-mod=ModList/@CF;ModList/@Dabs Framework;ModList/@VPPAdminTools;\"");
        var service = new BatchFileService(fs);

        IReadOnlyList<string> result = service.ReadModList(BatPath);

        Assert.Equal(new[] { "ModList/@CF", "ModList/@Dabs Framework", "ModList/@VPPAdminTools" }, result);
    }

    [Fact]
    public void ReadModList_ParsesMultiLineBlock()
    {
        var fs = new FakeFileSystem();
        string content = string.Join('\n', ExpectedModListLines(13));
        fs.AddFile(BatPath, content);
        var service = new BatchFileService(fs);

        IReadOnlyList<string> result = service.ReadModList(BatPath);

        Assert.Equal(ModPaths(13), result);
    }

    [Fact]
    public void ReadModList_ParsesLegacyBareNames()
    {
        var fs = new FakeFileSystem();
        fs.AddFile(BatPath, "set \"modList=-mod=@CF;@Dabs Framework;@VPPAdminTools;\"");
        var service = new BatchFileService(fs);
